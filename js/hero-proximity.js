...
const HeroProximity = (() => {
  ...
  function getApproachState() {
    ...
    if (!hero) return { text: '慢些翻页，林中魂物自会靠近', level: 'idle' };

    const targets = collectTargets();
    if (!targets.length) {
      return { text: '再翻一页，残页与魂物在等你', level: 'none' };
    }
    ...
    if (nearest < nearestWave * 0.65) {
      return { text: '停一停——契约将至，击掌之时', level: 'touch' };
    }
    if (nearest < nearestWave) {
      return { text: '她抬手了。故事在向你靠近', level: 'wave' };
    }
    if (nearest < nearestWave + 120) {
      return { text: '再微调一丝滚动，便是相遇', level: 'almost' };
    }
    if (nearest < nearestWave + 280) {
      return { text: '慢滚如翻页，把魂物送到她身旁', level: 'near' };
    }
    return { text: '蛋兔候在左侧，等一页风把她送来', level: 'far' };
  }
